...
public class Faktura_wydanie {
	...
	private JTextField text_adresdos;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Faktura_wydanie window = new Faktura_wydanie();
	...
	private void initialize() {
		...
		btnZaaduj.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try (Connection con = DriverManager.getConnection(url, user, password);
						PreparedStatement pst = con.prepareStatement("SELECT max(id_faktura_wyd) from faktura_wyd")) {
				...
				try (Connection con = DriverManager.getConnection(url, user, password);
						PreparedStatement pst = con.prepareStatement(
								"SELECT w.id_wyd,p.nazwa,p.opis,p.cena,w.ilosc,sum(p.cena*w.ilosc) FROM produkt p \r\n"
										+ "JOIN wydanie w ON w.id_prod_wyd = p.id_prod where w.id_faktura_wydanie ="
										+ maksymalna + " GROUP BY  p.nazwa,p.opis,p.cena,w.ilosc,w.id_wyd;")) { // where
																												// id_zamowienie_prz
																												// =
																												// "+maksymalna
				...
				try (Connection con = DriverManager.getConnection(url, user, password);
						PreparedStatement pst = con
								.prepareStatement("SELECT * from faktura_wyd where id_faktura_wyd = " + maksymalna)) {
				...
				try (Connection con = DriverManager.getConnection(url, user, password);
						PreparedStatement pst = con.prepareStatement("SELECT f.nazwa,f.adres,f.nip from faktura_wyd fw "
								+ "JOIN wydanie p ON p.id_faktura_wydanie = fw.id_faktura_wyd "
								+ "JOIN zamowienie z ON z.id_zamowienie = p.id_zamowienie_wyd "
								+ "JOIN firma f ON f.id_firma = z.id_firma_zam where fw.id_faktura_wyd = "
								+ maksymalna)) {
		...
		btnDodajUwagi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (Connection con = DriverManager.getConnection(url, user, password);
						PreparedStatement pst = con.prepareStatement("UPDATE  faktura_wyd SET uwagi= '"
								+ textuwagi.getText() + "' where id_faktura_wyd = " + maxxxx)) {
		 ...
		 * Powrót do wyborów
		 */
		JButton btnNewButton = new JButton("Do wyborów");
		...
		btnNewButton.setBounds(790, 658, 122, 23);
